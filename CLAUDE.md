...
**Runtime**: runs are incremental — reports already captured in `ofsted_csc_ilacs_history.csv` (with the current `PARSER_VERSION`) are reused without downloading or parsing the PDF, so a routine run costs ~153 provider-page HTML fetches plus real work only for LAs with a genuinely new inspection. A run with no usable history (missing/stale file, or `force_refresh = True`) re-parses everything — every captured report for every LA, not just the most recent — which is a one-off backfill on the order of 15-25 min (mostly reading PDFs already on disk under `export_data/inspection_reports/`, downloading only what's missing). There used to be a `pdf_data_capture` config flag for a faster link-list-only mode, but it was removed — it produced a DataFrame missing columns that the rest of the pipeline unconditionally assumed existed, so it crashed rather than actually working. If a faster local dev loop is needed, prefer capping `max_results` (process fewer LAs through the *same* full pipeline).
...
3. **Per-provider extraction** (`process_provider_links`) — for each LA provider page, finds all published inspection PDFs and identifies "children's services inspection" reports via the link's accessible/nonvisual text (fragile: depends on Ofsted's current markup/wording). **Every** matching report is captured, not just the most recent: each becomes a row in the inspection history, and the first (most recent) additionally feeds the summary outputs. Per report, the incremental check runs first — a `(urn, inspection_link)` already in the history store with the current `PARSER_VERSION` is reused outright (no download, no parse; this is what makes routine runs fast). Otherwise the PDF bytes come from disk if the file already exists under `export_data/inspection_reports/<urn>_<la_name>/`, else are downloaded and archived there. The call into PDF parsing (step 4) is wrapped in `try/except` — if extraction throws for one report, it gets logged (`logging.error`, with URN/name/filename) and placeholder `"data_unreadable"` values instead of aborting the whole run; note the placeholder row is still cached under the current `PARSER_VERSION`, so broken PDFs aren't pointlessly re-parsed daily (bump `PARSER_VERSION` or set `force_refresh` after a parser fix to retry them).

4. **PDF parsing** (`extract_inspection_data_update`, `fix_invalid_judgement_table_structure`, `fix_misalligned_judgement_table`) — this is the messiest and most change-prone part: pulls inspection dates, inspector name, framework type, and the judgement grades (overall effectiveness, impact of leaders, help & protection, in care, care leavers) out of inconsistently-formatted PDF tables via `tabula`/`PyPDF2`. Ofsted's report layout has changed over time — a Jan 2023 summary restructuring (`in_care_grade`/`care_leavers_grade` replace the older combined `care_and_care_leavers_grade`) and, more significantly, an ~April 2026 reform that dropped the overall effectiveness judgement from ILACS reports entirely (confirmed against real post-reform reports; the 4 remaining sub-judgements keep the same order, nothing renamed/reordered) — so this code has version-specific branching. `fix_misalligned_judgement_table` detects the post-reform case specifically (exactly 4 grades found, no "overall effectiveness" row) and sets `not_reported_post_reform` rather than treating it as a parse failure; this is a distinct, growing, and expected state, not one of the known per-LA extraction bugs (see README "Known limitations" — southend-on-sea, nottingham, redcar and cleveland, knowsley, stoke-on-trent), which are genuine bugs affecting a fixed small set of LAs. If `tabula` finds no table at all, or a report yields fewer grade rows than expected, this degrades to placeholder `"data_unreadable"`/`NaN` values (logged via `logging.warning`) rather than raising — see the `known_judgements`-based placeholder pattern shared across these functions.
...
The **summary** (the `.xlsx` and `index.html`): one row per Local Authority (153 rows currently), keyed on `urn`, describing that LA's **most recent published** ILACS inspection. The `.xlsx` is the full table (20 columns); `index.html` shows a trimmed, presentation-formatted subset (see `column_order` near the end of the script) with hyperlinked report/URN columns and title-cased text.

The **history** (`ofsted_csc_ilacs_history.csv`): one row per captured inspection report — `(urn, inspection_link)` keyed, all vintages back to ~2015 for some LAs — carrying the raw per-report scrape fields (`history_columns` in the script) plus `pdf_filename` and `parser_version`, un-enriched. Older-format reports (pre-2018 SIF era especially) parse best-effort and may hold `data_unreadable`/blank grades — expected, logged, not treated as failures by CI.
...
- Some LAs have PDF encoding/formatting quirks causing specific columns to fail extraction (see README "Known limitations"); when fixing one, check whether the fix regresses the others, as several fixes have been layered defensively (`fix_invalid_judgement_table_structure`, `fix_misalligned_judgement_table`).
